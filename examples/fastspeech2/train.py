...
class SaveCallBack(Callback):
    ...
    def step_end(self, run_context):
        ...
        losses = (
            net.total_loss.asnumpy(),
            net.mel_loss.asnumpy(),
            net.duration_loss.asnumpy(),
            net.pitch_loss.asnumpy(),
            net.energy_loss.asnumpy(),
        )
        ...
        for module, name in zip(
            [self.model, self.optimiser], [model_save_name, optimiser_save_name]
        ):
...
def main():
    ...
    if os.path.exists(args.restore):
        ...
        loaded = ms.load_checkpoint(
            args.restore.replace("model", "optimiser"),
            optimiser,
            filter_prefix="learning_rate",
        )
    ...
    scale_sense = nn.DynamicLossScaleUpdateCell(
        loss_scale_value=2**10, scale_factor=2, scale_window=1000
    )
    network = MyTrainOneStepCell(
        model, optimiser, scale_sense, max_grad_norm=hps.max_grad_norm
    )
