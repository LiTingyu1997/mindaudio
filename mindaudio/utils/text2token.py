...
def get_parser():
    ...
    parser.add_argument(
        "--skip-ncols", "-s", default=1, type=int, help="skip first n columns"
    )
    ...
    parser.add_argument(
        "text",
        type=str,
        default="/aishell_transcript_v0.8.txt",
        nargs="?",
        help="input text",
    )
    parser.add_argument(
...
def get_matched_positions(a, rs):
...
def cn_char_en_bpe(a, sp):
    ...
    for j in b:
        ...
        for line in j.strip().split("▁"):
            if not line.encode("UTF-8").isalpha():
                a.append(line)
            else:
                for k in sp.encode_as_pieces(line):
                    a.append(k)
    return a
