...
class FastSpeech2(nn.Cell):
    def __init__(self, hps):
        ...
        self.mel_linear = nn.Dense(
            hps.model.transformer.decoder_hidden, hps.n_mels
        ).to_float(self.dtype)
    ...
    def forward_expanded(
        ...
        expanded_output = self.expanded_encoder(
            expanded_phonemes, mel_masks, positions_decoder
        )
        ...
        if self.speaker_emb is not None:
            ...
            expanded_output = expanded_output + self.speaker_emb(speakers).unsqueeze(
                1
            ).expand(-1, max_mel_len, -1)
    ...
    def infer(
        ...
        yh2 = self.variance_adaptor.infer_emb_frame_level(
            yh["output"], mel_masks, p_control, e_control
        )
...
class FastSpeech2WithLoss(FastSpeech2):
    def __init__(self, loss_fn, *args, **kwargs):
        ...
        self.total_loss = ms.Parameter(
            ms.Tensor(0, dtype=self.dtype), requires_grad=False
        )
        self.mel_loss = ms.Parameter(
            ms.Tensor(0, dtype=self.dtype), requires_grad=False
        )
        self.duration_loss = ms.Parameter(
            ms.Tensor(0, dtype=self.dtype), requires_grad=False
        )
        self.pitch_loss = ms.Parameter(
            ms.Tensor(0, dtype=self.dtype), requires_grad=False
        )
        self.energy_loss = ms.Parameter(
            ms.Tensor(0, dtype=self.dtype), requires_grad=False
        )
        self.mel_predictions = ms.Parameter(
            ms.Tensor(np.ones([1, 742, 128]), dtype=self.dtype), requires_grad=False
        )
